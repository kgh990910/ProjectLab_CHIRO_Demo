...
def calculate_curvature_and_direction(left_fit, right_fit, img_shape):
    ploty = np.linspace(0, img_shape[0]-1, img_shape[0])
    y_eval = np.max(ploty)

    # 곡률 반경 계산을 위한 2차 다항식의 A와 B 계수 정의
    left_A = left_fit[0]
    left_B = left_fit[1]
    right_A = right_fit[0]
    right_B = right_fit[1]
    ...
    # 차선의 중간 지점 계산
    left_lane_bottom_x = left_A*y_eval**2 + left_B*y_eval + left_fit[2]
    right_lane_bottom_x = right_A*y_eval**2 + right_B*y_eval + right_fit[2]
    lane_center = (left_lane_bottom_x + right_lane_bottom_x) / 2

    # 차량 위치 (일반적으로 이미지 중앙)
    vehicle_position = img_shape[1] / 2

    # 차선 중심과 차량 위치 차이
    lane_deviation = vehicle_position - lane_center

    # 곡률 방향 결정 (양수는 오른쪽, 음수는 왼쪽)
    direction = "left" if lane_deviation > 0 else "right"

    # 평균 곡률 반경 반환
    curvature = (left_curverad + right_curverad) / 2
    return curvature, direction

def calculate_steering_angle(curvature, direction, curvature_scale_factor=1.0):
    # 스티어링 각도 계산
    base_angle = curvature_scale_factor / curvature
    steering_angle = base_angle if direction == "right" else -base_angle
...
def draw_lane(img_src, img_warped, pts1, pts2, ret):
    ...
    #곡률계산
    curvature, direction = calculate_curvature_and_direction(ret[0], ret[1], img_src.shape)
    steering_angle = calculate_steering_angle(curvature, direction)
    print("Calculated steering angle:", steering_angle)
    


    mean_lane = np.mean([ret[0], ret[1]], axis=0)
    # 이미지의 중앙 위치
    image_center = img_src.shape[1] / 2

    # 중앙 차선과 이미지 중앙의 차이 계산
    lane_deviation = mean_lane[-1] - image_center

    # 차선의 위치에 따라 조정할 방향 결정
    if lane_deviation > 20:  # 차선이 오른쪽으로 치우침
        direction = "Left"
    elif lane_deviation < -20:  # 차선이 왼쪽으로 치우침
        direction = "Right"
    else:
        direction = "Straight"

    # 여기서 'direction' 변수를 사용하여 차량의 이동 방향을 조정할 수 있습니다.
    # 예: 차량 제어 시스템에 방향 전송 또는 콘솔에 출력
    print("Direction to adjust:", direction)
    # mean = 평균값 구하는 함수
    pts_mean = np.array([np.flipud(np.transpose(np.vstack([mean_lane, ret[2]])))])
    # vstack = 배열을 세로로 결합       transpose = 행과 열 바꾸기(전치행렬)
    ...
    # 1번 내용과 동일
    img_res = cv2.resize(img_res, (img_src.shape[1], img_src.shape[0]))
...
temp_ret = []
cap = cv2.VideoCapture("./sibal.mp4")
